module daoship::dao {
    ...
    #[view]
    public fun get_proposal_details(proposal_id: u64): (u64, address, u64, u64, u64, bool) acquires DAOConfig {
        let dao_config = borrow_global<DAOConfig>(@daoship);
        assert!(table::contains(&dao_config.proposals, proposal_id), error::not_found(EPROPOSAL_NOT_FOUND));
        let proposal = table::borrow(&dao_config.proposals, proposal_id);
        (proposal.id, proposal.creator, proposal.yes_votes, proposal.no_votes, proposal.total_votes, proposal.executed)
    }

    #[view]
    public fun get_vote_details(proposal_id: u64, voter: address): (address, u64, bool, u64, bool) acquires DAOConfig {
        let dao_config = borrow_global<DAOConfig>(@daoship);
        ...
        let vote = simple_map::borrow(votes_map, &voter);
        (vote.voter, vote.proposal_id, vote.vote, vote.voting_power, vote.claimed_reward)
    }
